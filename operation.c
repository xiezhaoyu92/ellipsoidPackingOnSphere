...
void vectorCopy(const double *x1, double *x2, const int n) {
    for(int i=0; i<n; i++)
        x2[i] = x1[i];
}

void directorCalc(double Rad, double *position, double *u) {
    double t1[3];
    double t2[3];
    double u1[3];
    double u2[3];
    tangent1(Rad, position, t1);
    tangent2(Rad, position, t2);
