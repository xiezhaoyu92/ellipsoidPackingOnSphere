...
//  main.c
//  ellipsoidalPacking
//  Packing ellipsoidal particles on the surface of a sphere
//
//  Created by Zhaoyu Xie on 10/26/17.
//  Copyright © 2017 Zhaoyu Xie. All rights reserved.
//

//#define checkUndoOverlap
...
int main(int argc, char * argv[]) {
    double R0 = 1; //radius of the large sphere
    ...
    int optIndex = 0;
    int opt;
    while ((opt = getopt_long(argc, argv,"a:b:n:R:ms:u",
    ...
    if(readUnfinishedQ){
        ...
        FILE *rFileTemp=NULL;
        rFileTemp = fopen("sphereParamsTemp.dat","r");
        if (rFileTemp) {
            fscanf(rFileTemp, "%lf", &R);
            fclose(rFileTemp);
        } else {
    ...
    for(int i=0; i<nPart[0]; i++)
        partBoundX[i] = -R0+i*2*a;
    for(int i=0; i<nPart[1]; i++)
        partBoundY[i] = -R0+i*2*a;
    ...
    if(readUnfinishedQ){
        ...
        for(int i=0; i<np; i++) {
            double u = conformalInvert(genrand_real1());
            double v = 2*PI*genrand_real2();
            p[i].position[0] = R*sqrt(1-u*u)*cos(v);
            p[i].position[1] = R*sqrt(1-u*u)*sin(v);
            p[i].position[2] = R*u;
            p[i].position[3] = PI*genrand_real2();
        
            directorCalc(R, p[i].position, p[i].director);
        ...
        undoOverlaps(p, maxUndoStepsStart, dtOverlap, &jammed, a, b, R, np, partBoundX, partBoundY,  partBoundZ, nPart);
        if (jammed) {
        ...
        for (int i=0; i<startupDiffusionSteps; i++) {
            ...
            for (int j=0; j<np; j++) {
                ...
                addStochasticForce(&(p[k]), DaStartup, DbStartup, DthStartup, R);
                gradDescentConstrainedIntegrate(&(p[k]), dtDiffusion, R);
    ...
    for (simTime = simTimeStart; simTime <= simTimeFinal && !jammed; simTime += dtDiffusion) {
        if (simTime > nextRelaxationTime) {
            ...
            for(int j=0; j<np; j++) {
                projectOntoSphere(&(p[j]), R);
                partitionOneParticle(&(p[j]), partBoundX, partBoundY, partBoundZ, nPart);
            }
            int nearJammed = FALSE;
            undoOverlaps(p, maxUndoSteps, dtOverlap, &nearJammed, a, b, R, np, partBoundX, partBoundY, partBoundZ, nPart);
            if(nearJammed) {
                R = ROld;
                for(int j=0; j<np; j++) {
                ...
                FILE *rFileTemp=NULL;
                rFileTemp = fopen("sphereParamsTemp.dat.tmp","w");
                if (rFileTemp) {
                ...
                system("mv -f sphereParamsTemp.dat.tmp sphereParamsTemp.dat");
        ...
        for(int i=0; i<np; i++)
            addStochasticForce(&(p[i]), Da, Db, Dth, R);
    ...
    FILE *rFile=NULL;
    rFile = fopen("sphereParams.dat","w");
    if (rFile) {
        fprintf(rFile, "%.15lf\n", R);
        fclose(rFile);
    } else {
